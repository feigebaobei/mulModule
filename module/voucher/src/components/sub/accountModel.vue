  ...
  <div class="account-model">
    ...
    <div class="account-model-wrapper" :style="modelStyles">
      <div class="model-title" v-if="hasHeader" :style="titleStyles">
        <slot name="header">
          <em v-if="stat == 1" class="el-icon-close" @click='closeModel($event)'></em>
          <em v-if="stat == 0" class="el-icon-error" @click='closeModel($event)'></em>
        </slot>
      </div>
      <div class="model-content">
        <slot name="content">
          <img v-if="stat == 1" src="../../../../../common/assets/img/voucher/suc.png">
          <img v-if="stat == 0" src="../../../../../common/assets/img/voucher/fail.png">
          <p class="mainContent">{{content}}</p>
        </slot>
      </div>
      <div class="model-footer" v-if="hasFooter">
        <slot name="footer">
          <button class="close-btn" @click='closeModel($event)'>确定</button>
        </slot>
      </div>
    </div>
  </div>
